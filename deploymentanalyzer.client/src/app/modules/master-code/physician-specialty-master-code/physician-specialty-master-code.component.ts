...
import { AddPhysicianSpecialtyMasterCodeComponent } from '../../shared/master-code/add-physician-specialty-master-code/add-physician-specialty-master-code.component';
...
@Component({
  selector: 'app-physician-specialty-master-code',
  templateUrl: './physician-specialty-master-code.component.html',
  styleUrl: './physician-specialty-master-code.component.css'
})
export class PhysicianSpecialtyMasterCodeComponent {

  mastercodes: MasterCode[] = [];
  currentUser: any;
  masterCodeCategory: string = 'Specialty_Physician';
  ...
  ngOnInit(): void {
    ...
    this.loadSpecialty_PhysicianMasterCodes();
  ...
  loadSpecialty_PhysicianMasterCodes() {
  ...
  openDialog(mastercode?: MasterCode): void {
    ...
    const dialogRef = this.dialog.open(AddPhysicianSpecialtyMasterCodeComponent, {
      width: '400px',
      data: {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.apiService.Delete(deleteMasterCode + result).subscribe((response: ApiResponse) => {
          if (response.status) {
            this.toasterService.showSuccess(response.message);
            this, this.loadSpecialty_PhysicianMasterCodes();
            //this.searchTextField = '';
          } else {
            this.toasterService.showError(response.message);
          }
