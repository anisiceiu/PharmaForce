...
import { AddTherapeuticCategoryMasterCodeComponent } from '../../shared/master-code/add-therapeutic-category-master-code/add-therapeutic-category-master-code.component';
...
@Component({
  selector: 'app-therapeutic-category-master-code',
  templateUrl: './therapeutic-category-master-code.component.html',
  styleUrl: './therapeutic-category-master-code.component.css'
})
export class TherapeuticCategoryMasterCodeComponent {

  mastercodes: MasterCode[] = [];
  currentUser: any;
  masterCodeCategory: string = 'Therapeutic_Category';
  ...
  ngOnInit(): void {
    ...
    this.loadTherapeuticCategoryMasterCodes();
  ...
  loadTherapeuticCategoryMasterCodes() {
  ...
  openDialog(mastercode?: MasterCode): void {
    ...
    const dialogRef = this.dialog.open(AddTherapeuticCategoryMasterCodeComponent, {
      width: '400px',
      data: {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.apiService.Delete(deleteMasterCode + result).subscribe((response: ApiResponse) => {
          if (response.status) {
            this.toasterService.showSuccess(response.message);
            this, this.loadTherapeuticCategoryMasterCodes();
            //this.searchTextField = '';
          } else {
            this.toasterService.showError(response.message);
          }
