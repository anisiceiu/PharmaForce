...
import { AddPhysicianFocusMasterCodeComponent } from '../../shared/master-code/add-physician-focus-master-code/add-physician-focus-master-code.component';
...
@Component({
  selector: 'app-physician-focus-master-code',
  templateUrl: './physician-focus-master-code.component.html',
  styleUrl: './physician-focus-master-code.component.css'
})
export class PhysicianFocusMasterCodeComponent {

  mastercodes: MasterCode[] = [];
  currentUser: any;
  masterCodeCategory: string = 'Physicians_Focus';
  ...
  ngOnInit(): void {
    ...
    this.loadPhysicians_FocusMasterCodes();
  ...
  loadPhysicians_FocusMasterCodes() {
  ...
  openDialog(mastercode?: MasterCode): void {
    ...
    const dialogRef = this.dialog.open(AddPhysicianFocusMasterCodeComponent, {
      width: '400px',
      data: {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.apiService.Delete(deleteMasterCode + result).subscribe((response: ApiResponse) => {
          if (response.status) {
            this.toasterService.showSuccess(response.message);
            this, this.loadPhysicians_FocusMasterCodes();
            //this.searchTextField = '';
          } else {
            this.toasterService.showError(response.message);
          }
