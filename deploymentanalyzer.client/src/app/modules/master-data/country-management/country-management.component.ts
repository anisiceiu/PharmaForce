...
export class CountryManagementComponent implements OnInit {
  ...
  categories: string[] = [];
  selectedCategory: string = 'Country';
  ...
  displayedColumns: string[] = ['select','name', 'category','region','currency_symbol'];
  ...
  openDialog(mastercode?: MasterCode): void {
    ...
    const dialogRef = this.dialog.open(AddEditMastercodePopupComponent, {
      ...
      data: { mastercode: mastercode ? mastercode :this.dataSource,
        categories: this.categories, masterCodeCategory: 'Country'
      }
    });
